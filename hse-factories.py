    ...
    with open("result/factories_rf.csv", "w", newline="", encoding="utf-8") as of:
        ...
        with requests.Session() as s:
            for page in range(pages):
                ...
                status_code, result = smart_http_request(
                    s, url=url, params=params, headers=headers
                )

                if status_code == 200:
                    teasers_soup = BeautifulSoup(result.text, "html.parser")

                    for factory_teaser in teasers_soup.find_all(
                        attrs={"class": re.compile("factory\steaser\steaser\-\d*")}
                    ):
                        factory_dict = {k: None for k in factory_attrs}
                        factory_title_tag = factory_teaser.find(
                            attrs={"class": "factory__head"}
                        ).find(attrs={"class": "factory__title"})
                        if factory_title_tag:
                            factory_dict["name"] = factory_title_tag.text.strip()
                            factory_dict["urn"] = factory_title_tag["href"]
                        factory_txt_tag = factory_teaser.find(
                            attrs={"class": "factory__txt"}
                        )
                        if factory_txt_tag:
                            lineage_tag = factory_txt_tag.find(
                                attrs={"class": "lineage-item lineage-item-level-0"}
                            )
                            if lineage_tag:
                                factory_dict["lineage"] = lineage_tag.text
                            people_tag = factory_txt_tag.find(
                                attrs={"class": "factory__people content-list__title"}
                            )
                            if people_tag:
                        ...
                        factory_categories_tags = factory_teaser.find(
                            attrs={"class": "factory__category"}
                        ).find_all("li")
                        if factory_categories_tags:
                            factory_dict["categories"] = ", ".join(
                                [
                                    x.find("a").text
                                    for x in factory_categories_tags
                                    if x.find("a")
                                ]
                            )
                        ...
                        if factory_dict.get("urn"):
                            url = (
                                f"https://xn--80aegj1b5e.xn--p1ai{factory_dict['urn']}"
                            )
                            status_code, result = smart_http_request(
                                s, url=url, headers=headers
                            )
                            if status_code == 200:
                                about_soup = BeautifulSoup(result.text, "html.parser")
                                factory_dict["descr"] = " ".join(
                                    [
                                        x.text
                                        for x in about_soup.find(
                                            attrs={"id": "company-descr"}
                                        ).find_all("p")
                                    ]
                                )
                                for content_list_item_tag in about_soup.find(
                                    attrs={"class": "content-list"}
                                ).find_all("li"):
                                    if (
                                        content_list_item_tag.find(
                                            attrs={"class": "content-list__title"}
                                        ).text
                                        == "Тип предприятия"
                                    ):
                                        factory_dict["type"] = (
                                            content_list_item_tag.find(
                                                attrs={"class": "content-list__descr"}
                                            ).text.strip()
                                        )
                                    if (
                                        content_list_item_tag.find(
                            ...
                            if status_code == 200:
                                ...
                                for content_list_item_tag in details_soup.find(
                                    attrs={"class": "content-list"}
                                ).find_all("li"):
                                    if (
                                        content_list_item_tag.find(
                            ...
                            if status_code == 200:
                                contacts_soup = BeautifulSoup(
                                    result.text, "html.parser"
                                )
                                for content_list_item_tag in contacts_soup.find(
                                    attrs={"class": "content-list"}
                                ).find_all("li"):
                                    if (
                                        content_list_item_tag.find(
                                ...
                                if contacts_soup.find(attrs={"class": "category-map"}):
                                    factory_dict["geojson"] = (
                                        contacts_soup.find(
                                            attrs={"class": "category-map"}
                                        )
                                        .find(attrs={"class": "geofield-ymap"})
                                        .get("data-map-objects")
                                    )

                        if factory_dict.get("geojson"):
                            feature = json.loads(factory_dict.get("geojson"))[
                                "features"
                            ][0]
